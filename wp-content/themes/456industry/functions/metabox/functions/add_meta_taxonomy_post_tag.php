<?php
function lpd_post_tag_add_new_meta_field() {
	wp_enqueue_media();
	?>
...
add_action( 'post_tag_add_form_fields', 'lpd_post_tag_add_new_meta_field', 10, 2 );

function lpd_post_tag_edit_meta_field($term) {
	
 	wp_enqueue_media();

	$t_id = $term->term_id;
 
	$term_meta = get_option( "post_tag_$t_id" );
...
add_action( 'post_tag_edit_form_fields', 'lpd_post_tag_edit_meta_field', 10, 2 );

function lpd_post_tag_save_custom_meta( $term_id ) {
	if ( isset( $_POST['term_meta'] ) ) {
		...
		update_option( "post_tag_$t_id", $term_meta );
	}
}  
add_action( 'edited_post_tag', 'lpd_post_tag_save_custom_meta', 10, 2 );  
add_action( 'create_post_tag', 'lpd_post_tag_save_custom_meta', 10, 2 );